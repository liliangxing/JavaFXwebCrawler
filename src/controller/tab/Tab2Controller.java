...
public class Tab2Controller implements CallBack {
    ...
    private void firstGet(String site) {
        ...
        if (fileName.contains(Constant.SEPARATOR)) {
            mustInclude = site.split(Constant.SEPARATOR)[0];
        ...
        for (Element link : links) {
            ...
            if (lin.startsWith(mustInclude) && !lin.endsWith("#")) {
                ...
                if (lin.substring(lin.lastIndexOf("/") + 1).contains(".")) {
                    try {
                        String[] mylinArr = lin.substring(lin.lastIndexOf("/") + 1, lin.lastIndexOf(".")).split(Constant.SEPARATOR);
                        String myLin;
    ...
    @FXML
    private synchronized void btn2searchClicked(ActionEvent event) throws IOException {
...
//		txt3.selectAll();
    ...
    @FXML
    private void btn2browseClicked(ActionEvent event) {
    ...
    private void btn2saveClicked(ActionEvent event) {
        ...
        boolean txtFlag =
                StringUtils.isBlank(txt3.getText()) && StringUtils.isNotBlank(txtArea.getText());
        if (txtFlag) {
            TextToImageLong.generateImage(txtArea.getText());
            TextToImage.generateImage(txtArea.getText());
            return;
        }
        int threadCount = 1;
            ...
            threadPool.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (file.exists() && file.length() > 80 * 1024) {
                            return;
                        } else {
                            file.delete();
                            file.createNewFile();
